...
#    M1  Live metrics sampler (eiciel-metrics.{sh,service,timer}) at 5s
#    M2  nft table inet eiciel_metrics for DNS query counters
#    M3  Daemon ops: metrics, metric-history
#    M4  Dashboard: live Coverage panel + Egress graph + Live/Apps panels
#    M5  Containment evidence includes metrics history.jsonl
#    M6  Hook 030 enables eiciel-metrics.timer
#    M7  Hook 040 verifies metrics files
#
#  All v1.7.1 fixes preserved: B1 (fail-closed reauth), B2 (autologin
#  off by default), F4 (no bare 'strings' package), F5 (tty-kill match).
# ═══════════════════════════════════════════════════════════════════
set -euo pipefail
...
# ── M1: live metrics sampler toggle ──
...
if [ "$REAUTH_ON" = "1" ] && [ "$GUI_AUTOLOGIN" = "1" ]; then
  echo "  ⚠️  GUI_AUTOLOGIN=1 with TOTP enabled: dashboard autostarts"
  echo "      on tty1 without an OS password prompt. Re-auth still gates"
  echo "      trigger/lift, but the dashboard is readable from the console."
  echo ""
fi
...
PRE_EOF
  ...
  # ─── index.html (v1.8 — live metrics + egress graph) ───
...
async function refreshState(){
  ...
  catch(e){s={daemon:'unreachable',watcher:'?',auditd:'?',fail2ban:'?',reauthConfigured:false};}
...
# ── M5: snapshot metrics history into evidence ──
...
# F5: 'who' emits 'tty1' / 'pts/0', not '/dev/tty1'
...
F2B_EOF
  ...
  # ── M2: nftables + metrics DNS counter table ──
  {
    cat <<'NFT_HEAD'
#!/usr/sbin/nft -f
flush ruleset
...
if [ -t 0 ]; then
  ...
  ║  GUI       : sudo eiciel-gui  (if not already running)   ║
...
EOF
  ...
  # ── Operator tools (persist-init, eiciel-tools) — unchanged ──
...
## 4. Live metrics
The sampler writes one JSON snapshot every 5 seconds to
/var/lib/eiciel-metrics/current.json and appends to history.jsonl.
Inspect from the dashboard's Coverage/Live panels, or:

    cat /var/lib/eiciel-metrics/current.json | jq .
    tail -n 20 /var/lib/eiciel-metrics/history.jsonl | jq .
...
EOF
  ...
  # ── M7: hook 040 verification includes metrics ──
...
if [ "${ENABLE_METRICS:-0}" = "1" ]; then
  ...
  # nftables.conf must define the metrics table for DNS counters
...
case "${1:-all}" in
  ...
  all)
    ...
    if [ "$REAUTH_ON" = "0" ] && [ "$LICENSE_ON" = "0" ]; then
      echo ""
      echo "  ⚠️  Both license gate and re-auth are OFF."
      echo "      trigger/lift will be refused by the daemon."
      echo "      This ISO is read-only from the dashboard."
    fi
    echo "═══════════════════════════════════════════════════════════════"
    ;;
