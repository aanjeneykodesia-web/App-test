...
# ─── main.js – full with all features ──────────────────────────
cat > main.js << 'EOF'
...
// ─── Self‑destruct on normal exit (not breach) ──────────────
...
// ─── NEW: IPC handlers for system/internet controls and self‑destruct on exit ──
...
function createWindow() {
  ...
  // When window is closed, schedule self‑destruct (unless in test mode)
...
# ─── index.html – full UI (copied from your original script) ──
...
npx javascript-obfuscator main.js --output main.obf.js \
  --compact true --self-defending true --string-array true --string-array-encoding 'base64' --control-flow-flattening false
