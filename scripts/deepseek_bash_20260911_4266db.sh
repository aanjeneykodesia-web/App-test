...
#  Produces:
#    ./eiciel-app-dist/EicielDashboard-linux-x64/   (Electron app)
#    ./eiciel-app-dist/eicield/eicield.py           (root daemon)
#    ./eiciel-app-dist/eicield/eicield.service      (systemd unit)
#    ./eiciel-app-dist/eicield/install-daemon.sh    (installer)
# ═══════════════════════════════════════════════════════════════════
set -euo pipefail
...
# ─────────────────────────────────────────────────────────────────
# 1. package.json  (Electron 33 = supported; no --no-prune)
# ─────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────
# 2. main.js — talks to eicield over a Unix socket. No sudo. No shell-outs.
# ─────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────
# 3. preload.js — unchanged surface
# ─────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────
# 4. index.html — same UI as before (kept identical to your original)
# ─────────────────────────────────────────────────────────────────
...
function badge(el,label,val){el.textContent=`${label}: ${val}`;const ok=val==='active'||val==='yes';el.classList.toggle('ok',ok);el.classList.toggle('bad',!ok);}
async function refreshState(){
  const s=await window.eiciel.state();
  badge($('badge-daemon'),'daemon',s.daemon||'unknown');
  badge($('badge-watcher'),'watcher',s.watcher||'unknown');
  badge($('badge-auditd'),'auditd',s.auditd||'unknown');
  badge($('badge-f2b'),'fail2ban',s.fail2ban||'unknown');
...
# ─────────────────────────────────────────────────────────────────
# 5. eicield.py — the root daemon
# ─────────────────────────────────────────────────────────────────
cat > daemon/eicield.py <<'DAEMON_EOF'
#!/usr/bin/env python3
"""eicield — root-owned Unix-socket daemon for the Eiciel dashboard.

Only process that touches /etc/eiciel, /var/lib/incidents, nft, systemctl,
journalctl, and the containment script. Peer verified via SO_PEERCRED.
"""
...
def audit():
    r = run("ausearch", ["-k", "priv_esc", "--start", "today", "-i"])
    if not r["ok"]:
        return {"ok": True, "text": r["stderr"] or "(no audit events)"}
    return {"ok": True, "text": "\n".join(r["stdout"].splitlines()[-100:])}

def trigger():
    # Run as a detached process so the client returns promptly.
    try:
...
# ─────────────────────────────────────────────────────────────────
# 7. install-daemon.sh — run on target (as root)
# ─────────────────────────────────────────────────────────────────
