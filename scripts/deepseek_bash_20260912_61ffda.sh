...
# ── INSTALL PATCH: build mode. live = boot-only ISO. install = add debian-installer.
...
# ── INSTALL PATCH: pick installer mode based on BUILD_MODE ──
...
# ── Hook 020 — admin user ──
# ── INSTALL PATCH: only lock password on live builds. Install builds leave it
#     unset so the debian-installer prompts for a password during install. ──
...
# ── MOTD ──
# ── INSTALL PATCH: mention the installer in the banner when BUILD_MODE=install ──
...
# ── Hook 060 — cleanup ──
# ── INSTALL PATCH: only strip docs/logs on live builds ──
...
# Only strip docs/logs for the live image. On an install target they should survive.
if [ "${BUILD_MODE:-live}" = "live" ]; then
...
# ── package list ──
# ── INSTALL PATCH: debian-installer-launcher added; included conditionally below ──
...
# ── INSTALL PATCH: append installer launcher only for install builds ──
if [ "$BUILD_MODE" = "install" ]; then
  cat >> config/package-lists/eiciel.list.chroot <<'EOF'
...
# ── INSTALL PATCH: export BUILD_MODE so chroot hooks can read it ──
BUILD_MODE="$BUILD_MODE" lb chroot 2>&1 | tee chroot.log
