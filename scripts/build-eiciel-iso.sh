#!/bin/bash
# ═══════════════════════════════════════════════════════════════════
#  Eiciel IR Console — ISO builder
#  (detection-and-response appliance, not a "protected server")
# ═══════════════════════════════════════════════════════════════════
set -euo pipefail
...
export BUILD_MODE ISOLATION_TIMEOUT REAUTH_TOTP_SECRET
...
MFA_SSH="${MFA_SSH:-1}"
MFA_SUDO="${MFA_SUDO:-1}"
export ENABLE_MFA MFA_SSH MFA_SUDO

echo "═══════════════════════════════════════════════════════════════"
echo "  Eiciel IR Console — build summary"
echo "  MGMT_IP         = $MGMT_IP"
echo "  WEBHOOK_URL     = $WEBHOOK_URL"
echo "  STOP_SERVICES   = ${STOP_SERVICES:-（empty）}"
echo "  BUILD_MODE      = $BUILD_MODE"
echo "  PERSISTENCE     = $ENABLE_PERSISTENCE"
echo "  LICENSE_GATE    = $LICENSE_GATE_MODE"
echo "  REAUTH          = $([ -n "$REAUTH_TOTP_SECRET" ] && echo enabled || echo disabled)"
echo "  ISOLATION_TTL   = ${ISOLATION_TIMEOUT:-manual-lift}"
echo "  ADMIN_CIDRS     = ${ADMIN_CIDRS:-any}"
echo "  ALLOWED_PORTS   = $ALLOWED_PORTS"
echo "  ENABLE_MFA      = $ENABLE_MFA (ssh=$MFA_SSH sudo=$MFA_SUDO)"
echo "  AUTO_UPDATES    = $ENABLE_AUTO_UPDATES"
echo "═══════════════════════════════════════════════════════════════"
...
if [ "$LICENSE_GATE_MODE" != "none" ]; then
  ...
  if [ -z "$LICENSE_PUBKEY_FILE" ] || [ ! -f "$LICENSE_PUBKEY_FILE" ]; then
    echo "❌ LICENSE_GATE_MODE=$LICENSE_GATE_MODE requires LICENSE_PUBKEY_FILE"
    exit 1
  fi
...
if [ -n "$ADMIN_CIDRS" ]; then
  for cidr in $ADMIN_CIDRS; do
    if ! printf '%s' "$cidr" | grep -qE '^([0-9]{1,3}\.){3}[0-9]{1,3}(/[0-9]{1,2})?$'; then
      echo "❌ ADMIN_CIDRS entry '$cidr' is not a valid IPv4 address or CIDR"
      exit 1
    fi
  done
fi
...
for port in $ALLOWED_PORTS; do
  if ! [[ "$port" =~ ^[0-9]{1,5}$ ]] || [ "$port" -lt 1 ] || [ "$port" -gt 65535 ]; then
    echo "❌ ALLOWED_PORTS entry '$port' is not a valid TCP port"
    exit 1
  fi
done
...
if [ -n "$REAUTH_TOTP_SECRET" ]; then
  ...
  echo "  re-auth TOTP  = enabled (secret in /etc/eiciel/reauth.secret)"
...
if command -v qrencode >/dev/null 2>&1; then
  qrencode -t ANSIUTF8 "otpauth://totp/Eiciel:admin@$(hostname)?secret=$SECRET&issuer=Eiciel"
else
...
# ── Watcher (dual-source, expanded trigger set) ──
...
logger -t eiciel "watch: starting (audit.log + journald, self-protection on)"
...
# ── Audit rules (Phase 0: self-protection) ──
...
# ── Self-protection: the detector watching itself ──
...
# Stopping auditd / the watcher is an incident
...
# Lifting isolation is an action that must be logged
...
# ── nftables (CIDR-restricted SSH if ADMIN_CIDRS set) ──
{
  cat <<'NFT_HEAD'
#!/usr/sbin/nft -f
flush ruleset
...
NFT_HEAD
  ...
  if [ -n "$ADMIN_CIDRS" ]; then
    for cidr in $ADMIN_CIDRS; do
      echo "        ip saddr $cidr tcp dport 22 accept"
    done
  else
...
## 2. SSH from MGMT_IP
ssh admin@<server>
# MFA required once enrolled. If not yet:
...
- Re-enroll MFA: rm ~/.google_authenticator && google-authenticator -t -d -f -r 3 -R 30 -w 3
- Rotate TOTP re-auth: sudo eiciel-reauth-show (re-scan on new device)
...
if [ -t 0 ]; then
  ...
  MFA_STATE="enrolled"
  [ -f "$HOME/.google_authenticator" ] || MFA_STATE="NOT enrolled (google-authenticator -t -d -f -r 3 -R 30 -w 3)"
  cat <<BANNER
...
if [ "$LICENSE_GATE_MODE" != "none" ]; then
  GATE_ENABLE="systemctl enable eiciel-license-gate.service"
else
  GATE_ENABLE=":"
fi
...
if grep -qE '^\s*tcp dport 22 accept' /etc/nftables.conf && \
   grep -qE '^\s*ip saddr' /etc/nftables.conf; then
  echo "  ⚠️  nftables has both CIDR-gated and blanket SSH rules"
fi
...
if [ "$ENABLE_MFA" = "1" ]; then
  echo "libpam-google-authenticator" >> config/package-lists/eiciel.list.chroot
fi
...
if [ "$BUILD_MODE" = "install" ]; then
  echo "debian-installer-launcher" >> config/package-lists/eiciel.list.chroot
fi
...
menuentry "Eiciel IR Console (recovery shell)" {
...
echo "      Isolation TTL   = ${ISOLATION_TIMEOUT:-manual}"
echo "      Admin CIDRs     = ${ADMIN_CIDRS:-any}"
echo "      Allowed ports   = $ALLOWED_PORTS"
echo "      MFA (ssh/sudo)  = $MFA_SSH/$MFA_SUDO"
