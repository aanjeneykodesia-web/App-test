#!/bin/bash
# ═══════════════════════════════════════════════════════════════════
#  Eiciel Server ISO — containment edition + hardening
# ═══════════════════════════════════════════════════════════════════
set -euo pipefail
...
# ── nftables default (hardened: CIDR-restricted SSH + explicit port allow) ──
{
  cat <<'NFT_HEAD'
#!/usr/sbin/nft -f
flush ruleset
...
## 2. SSH from MGMT_IP
ssh admin@<server>
# MFA is required once enrolled. If not yet enrolled:
...
if [ -t 0 ]; then
  ...
  MFA_STATE="enrolled"
  [ -f "$HOME/.google_authenticator" ] || MFA_STATE="NOT enrolled (run: google-authenticator -t -d -f -r 3 -R 30 -w 3)"
  cat <<BANNER
...
# nftables must not have a blanket "tcp dport 22 accept" if ADMIN_CIDRS is set.
