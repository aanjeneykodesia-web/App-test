#!/bin/bash
# ─── Build Eiciel OS – Strongest Free Protection ──
set -e
echo "🛠️  Building Eiciel OS with full protection..."
...
# ─── ORIGINAL main.js (will be compiled to bytecode) ────────────
cat > main.real.js << 'EOF'
// ─── Original main.js – will be compiled to bytecode ──────────
...
function createWindow() {
  ...
  // Load index.html from the temp directory
...
# ─── tests/app.spec.js (with test-timeout fix) ──────────────────
mkdir -p tests
...
test.describe('Security Bot Attacks', () => {
  let app, window;
...
echo "🔒 Compiling main.js to bytecode..."
npx bytenode --compile main.real.js
mv main.real.jsc main.jsc
...
const eulaResult = dlg.showMessageBoxSync({
  type: 'warning',
  title: 'License & Non‑Disclosure Agreement',
  message: 'Eiciel OS – Proprietary Software',
  detail: `By clicking "Accept" you agree to:
- NOT reverse engineer, decompile, or decrypt any part of this software.
- NOT distribute, copy, or modify this software.
- Treat this software as confidential trade secrets.
- Any violation will result in immediate legal action and liability for damages.`,
  buttons: ['I Accept', 'I Decline'],
  checkboxLabel: 'I have read and agree to the terms',
  checkboxChecked: false
});
if (eulaResult.response !== 0 || !eulaResult.checkboxChecked) {
  app.quit();
  process.exit(0);
}
...
function decryptBlob() {
  ...
  const encData = fs.readFileSync(ENC_PATH, { encoding: null }).slice(16);
...
try {
  ...
  dialog.showErrorBox('Decryption Failed', 'The software could not be loaded. Please contact support.');
  app.quit();
  process.exit(1);
}
...
require('bytenode');
require(path.join(tempDir, 'main.jsc'));
EOF

# ─── Obfuscate the loader ──────────────────────────────────────
echo "🔒 Obfuscating loader..."
npx javascript-obfuscator loader.js --output loader.obf.js \
  --compact true --self-defending true --debug-protection true \
  --control-flow-flattening true --string-array true --string-array-encoding 'base64'
mv loader.obf.js loader.js
...
const FILES = ['main.jsc', 'preload.js', 'index.html', 'package.json', 'playwright.config.js', 'tests'];
const PASSWORD = 'EICIEL-PROTECT-2026';
...
if [ -z "$CI" ]; then
  ...
  echo "⏭️ Skipping build in CI – only running tests."
fi
