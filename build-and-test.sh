#!/bin/bash
# ─── Build Eiciel OS – Protected with Demo & Playwright ──
set -e
echo "🛡️  Building protected Eiciel OS (with Demo & Playwright)..."
...
# ─── main.js – original with Demo window ──────────────────────
cat > main.js << 'EOF'
...
function openDemoWindow() {
  ...
  const demoUrl = process.env.DEMO_URL || 'https://your-demo-vm.com';
  ...
  demoWindow.loadURL(demoUrl);

  // Lock down the demo window
  demoWindow.webContents.on('context-menu', (e) => e.preventDefault());
  demoWindow.webContents.on('before-input-event', (event, input) => {
  ...
  demoWindow.webContents.on('will-navigate', (event, url) => {
    if (!url.startsWith(demoUrl)) {
      event.preventDefault();
      dialog.showErrorBox('Navigation Blocked', 'You may only navigate within the demo environment.');
    }
  });

  demoWindow.webContents.setWindowOpenHandler(() => {
    dialog.showErrorBox('Blocked', 'External windows are not allowed.');
    return { action: 'deny' };
  });
...
function createWindow() {
  ...
  // Demo handler
  ipcMain.handle('open-demo', () => { openDemoWindow(); });
}
...
# ─── index.html – with Demo icon ──────────────────────────────
# (We include the full UI with the demo icon added)
...
<style>
/* ─── Full CSS from your working script ─── */
...
<div id="desktopArea" style="display:none;">
  <div class="desktop">
    <div class="desktop-icons">
      ...
      <!-- NEW DEMO ICON -->
      <div class="icon" data-app="demo"><span class="icon-img">🖥️</span><span>Demo</span></div>
    </div>
    <!-- Main Window -->
    ...
    <div class="start-menu" id="startMenu">
      ...
      <div class="menu-item" data-app="demo"><span class="icon">🖥️</span> Demo</div>
...
# Obfuscate main.js
npx javascript-obfuscator main.js --output main.obf.js --compact true --self-defending true --debug-protection true --control-flow-flattening true --string-array true --string-array-encoding 'base64'
mv main.obf.js main.js

# Obfuscate preload.js
npx javascript-obfuscator preload.js --output preload.obf.js --compact true --self-defending true --debug-protection true --control-flow-flattening true --string-array true --string-array-encoding 'base64'
...
if [ -z "$CI" ]; then
  ...
  echo "⏭️ Skipping build in CI (set CI env to avoid build)."
fi

# ─── Post‑install dependencies inside the packaged app ──────
if [ -z "$CI" ]; then
  ...
  echo "✅ Done! Protected .exe with Demo & Playwright at D:/data/EicielOS/EicielOS-win32-x64/EicielOS.exe"
else
  echo "✅ Build script executed in CI – dependencies installed, tests can run with 'npx playwright test'."
fi
