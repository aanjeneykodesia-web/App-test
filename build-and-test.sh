#!/bin/bash
# ─── Build Eiciel OS – No CAPTCHA, Full HTML + Headless Tests ──
set -e
echo "🛠️  Building Eiciel OS – no CAPTCHA, full HTML with headless tests..."
...
cat > package.json << 'EOF'
{
  ...
  "scripts": {
    ...
    "build": "electron-packager . EicielOS --platform=win32 --arch=x64 --out=D:/data/EicielOS --overwrite"
  },
  "dependencies": {
...
async function triggerBreach(reason) {
  ...
  // Show lockdown overlay immediately
  if (mainWindow) {
...
ipcMain.handle('enable-test-mode', () => {
  ...
  // Send skip-login signal to renderer
  if (mainWindow) {
...
# ─── index.html – Full UI with login overlay, lockdown wall ────
...
test.beforeAll(async () => {
  ...
  // Enable test mode – this will trigger skip-login event
  await window.evaluate(() => window.api.enableTestMode());

  // Wait for desktop to appear (auto-login should happen)
...
# ─── Install dependencies (skip build if CI) ──────────────────
...
if [ -z "$CI" ]; then
  ...
  npm run build
  echo "✅ Done! Your executable is in: D:/data/EicielOS/EicielOS-win32-x64/EicielOS.exe"
else
  echo "⏭️ Skipping build because we are in CI – only running tests."
fi
