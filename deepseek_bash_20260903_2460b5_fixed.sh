#!/bin/bash
# ─── Build Eiciel OS – Protected with internet control & self-destruct ──
set -e
echo "🛡️  Building protected Eiciel OS (full, with internet control)..."
...
# ─── main.js – with internet control & self-destruct on exit ──
cat > main.js << 'EOF'
...
// ─── New handlers for system & internet control ──────────────
...
function createWindow() {
  ...
  // Initially block internet (if not test mode)
...
# ─── index.html – with browser‑open/close internet controls ──
...
function initApp() {
  ...
  function closeWindow(win){
    if(win === eiciel){
      ...
      // ─── Block internet when browser is closed ──
  ...
  function openApp(app){
    if(app === 'eiciel'){
      ...
      if(win){
        ...
        // ─── Allow internet when browser is opened ──
        if (app === 'browser') {
...
# 1. Obfuscate main.js with reduced settings
...
# 3. Create loader.js (with diagnostics)
cat > loader.js << 'EOF'
