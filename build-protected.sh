...
# ─── main.js – with navigation blockers and conditional sandbox ──
cat > main.js << 'EOF'
...
# ─── index.html – full UI (copy from your working build) ──────
...
# Set Node memory limit and use milder obfuscation to avoid OOM
...
npx javascript-obfuscator main.js --output main.obf.js \
  ...
  --control-flow-flattening false   # <-- disable heavy option
mv main.obf.js main.js
unset NODE_OPTIONS

# 2. Obfuscate preload.js (smaller, so keep heavy options)
...
# FIX: Wrap anti‑debug checks in EICIEL_TEST_MODE condition
cat > loader.js << 'EOF'
...
// Anti‑debug: skip when EICIEL_TEST_MODE is set (for test runners)
...
echo "🔧 To run tests, set environment variable EICIEL_TEST_MODE=1 before launching the app."
