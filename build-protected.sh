...
# ─── Patch the test file to include EICIEL_TEST_MODE ────────────
if [ -f "tests/security.spec.js" ]; then
  echo "📝 Patching tests/security.spec.js to set EICIEL_TEST_MODE..."
  node -e "
const fs = require('fs');
const path = 'tests/security.spec.js';
...
"
  ...
  echo "⚠️ tests/security.spec.js not found – skipping patch."
fi
...
# ─── main.js – full with system/internet controls ──────────────
cat > main.js << 'EOF'
...
// ─── System and internet control functions ──────────────────────
...
function enableSystemProcesses() {
  if (testMode) { testSpies.enableSystemProcesses++; return; }
...
function blockInternet() {
  if (testMode) { testSpies.blockInternet++; return; }
...
function allowInternet() {
  if (testMode) { testSpies.allowInternet++; return; }
...
// ─── NEW IPC Handlers for system/internet controls ──────────────
...
function createWindow() {
  ...
  // Apply initial lockdown if not in test mode
  if (!testMode) {
...
# ─── index.html – full UI with browser internet toggle ──────────
# We need to patch the JavaScript inside index.html to call allowInternet() when browser opens and blockInternet() when it closes.
# Since the full HTML is huge, we'll use sed to inject the calls into the existing functions.
# We'll create a base index.html (the same as before) and then patch it with sed.

# First, create the full index.html (same as the one from your original script)
...
# ─── Apply the internet‑toggle patch to index.html ──────────────
# We already added the calls inside closeWindow and openApp, so no need to patch.
...
# 1. Obfuscate main.js with reduced settings and increased memory
echo "🔒 Obfuscating main.js (with memory limit)..."
...
# 2. Obfuscate preload.js (smaller, keep heavy options)
...
# 3. Create loader.js – with NODE_PATH and diagnostics
cat > loader.js << 'EOF'
...
echo "📝 The test file 'tests/security.spec.js' has been patched to include the env variable."
