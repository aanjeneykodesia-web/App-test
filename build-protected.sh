#!/bin/bash
# ─── Build Eiciel OS – Protected (fixed internet control, IPC tests) ──
set -e
...
# ─── main.js (with internet-on-browser-open logic) ──────────────
cat > main.js << 'EOF'
...
# ─── index.html (full, with internet control logic) ─────────────
...
function initApp() {
  ...
  function closeWindow(win){
    if(win === eiciel){
      ...
      // ─── BLOCK INTERNET WHEN CLOSING BROWSER ───
  ...
  function openApp(app){
    if(app === 'eiciel'){
      ...
      if(win){
        ...
        // ─── ALLOW INTERNET WHEN OPENING BROWSER ───
        if (app === 'browser') {
...
# 2. Create loader.js with encryption
cat > loader.js << 'EOF'
