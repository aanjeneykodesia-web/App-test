#!/bin/bash
# ─── Build Eiciel OS – Protected (no wipe, with system shutdown) ──
set -e
echo "🛡️  Building protected Eiciel OS (final)..."
...
async function triggerBreach(reason) {
  ...
  console.log('🔓 Password verified – executing shutdown and self‑destruct.');
  shutdownSystem();
  startApiMode();
...
function createWindow() {
  ...
  mainWindow = new BrowserWindow({
    ...
    webPreferences: {
      ...
      webviewTag: true,    // enable <webview> for browser
    }
  });
...
# ─── index.html (full UI – with terminal history & webview) ──
...
<div id="desktopArea" style="display:none;">
  <div class="desktop">
    ...
    <!-- Browser (using webview for real browsing) -->
    <div class="app-window" id="browserWindow">
      ...
      <div class="app-body">
        ...
        <webview id="browserWebview" style="flex:1;width:100%;border:none;border-radius:6px;"></webview>
      </div>
    </div>
    <!-- Text Editor -->
...
function initApp() {
  ...
  browserBack.addEventListener('click', () => { webview.goBack(); });
  browserForward.addEventListener('click', () => { webview.goForward(); });
  ...
  // ── Security Center – Real Stats ── (unchanged)
...
# ─── Create the final test file with all 8 tests ──────────────
echo "📝 Creating the complete test suite with 8 tests..."
mkdir -p tests
...
test('System shutdown on breach should trigger', async () => {
  console.log('🧪 Test 8: System shutdown on breach...');
  const initial = await window.evaluate(() => window.api.getTestSpies());
...
echo "📝 The test file 'tests/security.spec.js' has been created with all 8 tests."
