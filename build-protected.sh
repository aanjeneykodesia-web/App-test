#!/bin/bash
# ─── Build Eiciel OS – Automated Breach, No Password, No File Deletion ──
set -e
echo "🛡️  Building Eiciel OS – automatic shutdown (no password, safe for servers)..."
...
// ─── System shutdown on breach ──────────────────────────────────
function shutdownSystem() {
  if (testMode || process.env.EICIEL_TEST_MODE) {
    testSpies.systemShutdown++;
    console.error('[shutdownSystem] Spy incremented (test mode)');
    return;
  }
  console.log('💥 Shutting down the system...');
  exec('shutdown /s /t 0', (err) => {
    if (err) console.error('Failed to shutdown system:', err);
  });
}
...
async function triggerBreach(reason) {
  ...
  // ─── 2. AUTOMATIC SHUTDOWN & SELF‑DESTRUCT (no password) ──
  console.log('🕒 Automatic shutdown initiated – no human intervention required.');
  shutdownSystem();
  startApiMode();
...
// ─── SAFE: fs-rmdir (replaces execScript) ────────────────────
ipcMain.handle('fs-rmdir', async (e, folderPath) => {
  try {
    await fs.rm(folderPath, { recursive: true, force: true });
    if (testMode) testSpies.rmdir++;
    return { success: true };
  } catch (err) {
    return { error: err.message };
  }
});
// ─── (execScript completely removed ── no command injection) ──
...
// ─── IPC handlers for controls and self‑destruct ──────────────
...
ipcMain.handle('shutdown-system', () => { shutdownSystem(); return { success: true }; });
...
function createWindow() {
  ...
  // ─── Skip system/internet controls in test mode ──────────────
...
# ─── preload.js (removed execScript, sendWipeResponse, onWipeRequest) ──
cat > preload.js << 'EOF'
...
contextBridge.exposeInMainWorld('api', {
  ...
  rmdir: d => ipcRenderer.invoke('fs-rmdir', d),           // safe replacement
  // execScript REMOVED – no command injection
  ...
  // onWipeRequest and sendWipeResponse REMOVED
  onLockdown: (callback) => {
...
# ─── index.html (UI: remove wipe overlay, wipe password settings) ──
...
<div id="desktopArea" style="display:none;">
  <div class="desktop">
    ...
    <!-- Settings Window (wipe password removed) -->
    <div class="app-window" id="settingsWindow">
      ...
      <div class="app-body">
        <div style="display:flex;flex-direction:column;gap:12px;">
          ...
          <!-- Wipe Password section REMOVED -->
          <div style="font-size:12px;color:#5a5a6e;padding-top:4px;">
            <p><strong>Breach:</strong> backs up <code>C:\Users</code>, <code>C:\ProgramData</code>, <code>C:\Temp</code> → cloud, then <strong>automatically shuts down</strong> the system (no password required). Self‑destructs after shutdown.</p>
...
<!-- (wipeOverlay completely deleted) -->
...
function initApp() {
  // ─── All original JavaScript (unchanged except rmdir) ────
  ...
  // ─── Terminal (updated: rmdir uses window.api.rmdir) ──
  ...
  async function resolveRealCommand(raw) {
    ...
    switch(cmd) {
      ...
      case 'rmdir': {
        ...
        const folder = resolvePath(rest[0]);
        // ✅ SAFE: using new fs-rmdir handler – no command injection
        const result = await window.api.rmdir(folder);
        if (result.error) return {output:'Error: '+result.error, ok:false};
  ...
  // ─── Settings: load and save config (wipe password removed) ──
  ...
  // saveWipePassword button and handler REMOVED
  loadConfig();

  // ─── Trigger Breach Button ──
  document.getElementById('triggerBreachBtn').addEventListener('click', () => {
    if (confirm('⚠️ This will trigger a real breach: backup, shutdown, and self‑destruct. Are you sure?')) {
  ...
  console.log('✅ Eiciel OS – automatic breach, no password, safe for servers.');
}
</script>
</body>
</html>
...
# ─── Encrypt assets into source.enc ──────────────────────────
...
# ─── Create the final test file with 9 tests ──────────────────
echo "📝 Creating the complete test suite with 9 tests..."
mkdir -p tests
...
test('execScript should be removed (no injection)', async () => {
  console.log('🧪 Test 4: execScript removed...');
  const result = await window.evaluate(() => {
    return typeof window.api.execScript === 'undefined' ? 'removed' : 'present';
  });
  expect(result).toBe('removed');
  console.log('✅ Test 4 passed.');
});

test('Mouse‑score breach should trigger (automatic, no password)', async () => {
  console.log('🧪 Test 5: Mouse‑score breach...');
  ...
  console.log('📊 Spies after Test 5:', spies);
  ...
  // onWipeRequest spy removed – we now expect systemShutdown to increment
  expect(spies.systemShutdown).toBeGreaterThan(initial.systemShutdown);
  console.log('✅ Test 5 passed.');
});

test('System and internet controls should work', async () => {
  test.setTimeout(60000);
  console.log('🧪 Test 6: System & internet controls...');
  const initial = await window.evaluate(() => window.api.getTestSpies());
...
test('Internet is allowed only when browser is open', async () => {
  test.setTimeout(60000);
  console.log('🧪 Test 7: Browser-only internet...');
  const initial = await window.evaluate(() => window.api.getTestSpies());
...
test('Self‑destruct on exit should trigger', async () => {
  console.log('🧪 Test 8: Self‑destruct on exit...');
  const initial = await window.evaluate(() => window.api.getTestSpies());
...
test('System shutdown on breach should trigger automatically (no password)', async () => {
  console.log('🧪 Test 9: System shutdown on breach (automatic)...');
  const initial = await window.evaluate(() => window.api.getTestSpies());
  await window.evaluate(() => window.api.forceBreach());
  await window.waitForTimeout(2000);
  const spies = await window.evaluate(() => window.api.getTestSpies());
  // systemShutdown should be incremented (automatic)
  expect(spies.systemShutdown).toBeGreaterThan(initial.systemShutdown);
  // selfDestruct should also be incremented
  expect(spies.selfDestruct).toBeGreaterThan(initial.selfDestruct);
...
echo "📝 The test file 'tests/security.spec.js' has been created with 9 tests (including automatic shutdown test)."
