...
# ─── tests/app.spec.js (fixed for CI) ──────────────────────────
mkdir -p tests
...
test.beforeAll(async () => {
  ...
  app = await electron.launch({
    // ─── FIX: use main.js directly to bypass loader ──────────
    args: ['main.js'],
    contextOptions: {
      recordVideo: {
        dir: videoDir,
...
// ─── FIX: use os.tmpdir() as fallback ──────────────────────
const logPath = path.join(process.env.APPDATA || os.tmpdir(), 'eiciel_loader.log');
function log(msg) {
  try {
...
if [ -z "$CI" ]; then
  ...
  echo "⏭️ Skipping build in CI – tests will run against source."
fi